...
__global__ void node_reset(int *n_table, int *e_table, int *source, int *sink, int *N_NUM, int *E_NUM) {
...
__global__ void trace_cu(int *n_table, int *e_table, int *flg3, int *que3, int *N_NUM, int *E_NUM, int *cnt, int *flg1) {
  ...
  for (;;) {
    if (edge_id == -1) return;
    int lok_node = e_table[EADR(e_num[0], edge_id, look_n)];
    if (e_table[EADR(e_num[0], edge_id, FLOW)] > 0) {
      if (n_table[NADR(n_num[0], lok_node, STATE)] == 0) {
      	old = atomicExch(&(n_table[NADR(n_num[0], lok_node, STATE)]), state);
      	if (old == 0) {
      	  n_table[NADR(n_num[0], lok_node, HEIGHT)] = n_table[NADR(n_num[0], node_id, HEIGHT)] + 1;
      	  flg1[0] = 1;
      	}
      	else n_table[NADR(n_num[0], lok_node, STATE)] = old;
      }
...
__device__ void flow_t(int *e_table, int flow, int pre_edge, int edge, int N_NUM, int E_NUM) {
  ...
  for (;;) {
    if (target == -1) break;
    atomicSub(&e_table[EADR(E_NUM, target, FLOW)], flow);
    int reverse = e_table[EADR(E_NUM, target, REVERSE)];
    atomicAdd(&e_table[EADR(E_NUM, reverse, FLOW)], flow);
    target = e_table[EADR(E_NUM, target, ROUTE)];
  }
}
...
__global__ void aug_cu(int *n_table, int *e_table, int *flg1, int *que1, int *source, int *sink, int *e_flg, int *flow_sum, int *N_NUM, int *E_NUM) {
  ...
  for (;;) {
    if (n_table[NADR(n_num[0], node, STATE)] == 1) {
      root = IN_ROOT;
      look_n = IN_NODE;
      link = IN_LINK;
    }
    ...
    for (;;) {
      if (edge_id == -1) return;
      int flow1 = e_table[EADR(e_num[0], edge_id, FLOW)];
      int lok_node = e_table[EADR(e_num[0], edge_id, look_n)];
      if (flow1 > 0) {
	if (n_table[NADR(n_num[0], lok_node, HEIGHT)] == (n_table[NADR(n_num[0], node, HEIGHT)] - 1)) {
	  if (n_table[NADR(n_num[0], lok_node, STATE)] == n_table[NADR(n_num[0], node, STATE)]) {
	    if (lok_node == sink[0]) {
	      old = atomicExch(&e_flg[edge_id], 1);
	      if (old == 0) {
		flow = ((flow > flow1) ? flow1 : flow);
		flow_t(e_table, flow, pre_edge, edge_id, n_num[0], e_num[0]);
...
__global__ void edge_reset(int *e_table, int *e_flg, int *E_NUM) {
  int total_id = blockDim.x * blockIdx.x + threadIdx.x;
  __shared__ int e_num[1];
  e_num[0] = E_NUM[0];
  if (total_id >= e_num[0]) return;
  e_flg[total_id * 2] = 0;
  e_flg[total_id * 2 + 1] = 0;
  e_table[EADR(e_num[0], (total_id * 2), ROUTE)] = -1;
  e_table[EADR(e_num[0], (total_id * 2 + 1), ROUTE)] = -1;
}
...
int main(int argc, char **argv) {
  ...
  gpuErrchk( cudaMalloc((void**)&de_flg, E_SIZE) );
  ...
  for (;;) {
    ...
    flg_reset<<<1, 1>>>(d_cnt);
    node_reset<<<(N_NUM[0] / 64) + 1, 64>>>(nd_table, ed_table, d_source, d_sink, DN_NUM, DE_NUM);
    ...
    for (;;) {
      ...
      gpuErrchk( cudaThreadSynchronize() );
      trace_cu<<<(N_NUM[0] / 64) + 1, 64>>>(nd_table, ed_table, d_flg3, d_que3, DN_NUM, DE_NUM, d_cnt, d_flg1);
      gpuErrchk( cudaThreadSynchronize() );
    ...
    //augmentation stage
    for (int i = 0; i < 2; i++) {
      edge_reset<<<(E_NUM[0] / 64) + 1, 64>>>(ed_table, de_flg, DE_NUM);
      gpuErrchk( cudaThreadSynchronize() );
      aug_cu<<<flg3[0] / 64 + 1, 64>>>(nd_table, ed_table, d_flg3, d_que3, d_source, d_sink, de_flg, dflow_sum, DN_NUM, DE_NUM);
      gpuErrchk( cudaThreadSynchronize() );
    }
